...
print (sample['source_img'].size())
print (sample['target_img'].size())
print (sample['target_pose'].size())
print (sample['pose_mask'].size())

pose_mask = sample['pose_mask']
...
pose_mask = torch.cat((pose_mask1, pose_mask2, pose_mask3), dim = 1)
print(pose_mask.size())

target_img = sample['target_img']

img_after_mask = target_img*pose_mask

for i in range (batch_size):
    img = img_after_mask[i].numpy().transpose([1, 2, 0])
    io.imsave('/home/yiming/code/debug_temp/' + str(i+1) + '.jpg', img)
...
for epoch in range(epoch_num):
    ...
    for i_train, train_sample in enumerate(dataloader_train):
        ...
        if (i_train == 0) or (i_train+1) % 10 == 0:
            print("iteration: %d"%(i_train+1))
            if i_train == 0:
                record_loss = running_loss
            else:
                record_loss = running_loss / 10
